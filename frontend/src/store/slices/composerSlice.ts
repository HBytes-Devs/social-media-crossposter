...
type ComposerState = {
  ...
  autoGeneratedImageUrl: string | null;
  lastAutoImageContent: string | null;
  error: string | null;
  success: string | null;
  initialized: boolean;
};
...
const initialState: ComposerState = {
  ...
  autoGeneratedImageUrl: null,
  lastAutoImageContent: null,
  error: null,
  success: null,
  initialized: false,
};
...
export const generatePostImage = createAsyncThunk(
  ...
  async (
    ...
    try {
      const result = await api.generatePostImage(token, {
      ...
      return {
        ...
        replaceUrl: args.replaceUrl ?? null,
      };
    } catch (err) {
      return rejectWithValue(
        err instanceof Error ? err.message : "Image generate nahi ho saki",
      );
    }
  },
);
...
const composerSlice = createSlice({
  ...
  reducers: {
    ...
    removeImage(state, action: PayloadAction<string>) {
      const url = action.payload;
      state.images = state.images.filter((u) => u !== url);
      if (state.autoGeneratedImageUrl === url) {
        state.autoGeneratedImageUrl = null;
        state.lastAutoImageContent = null;
      }
      state.success = null;
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchAccounts.fulfilled, (state, action) => {
        ...
        state.selectedAccounts = state.selectedAccounts.filter((id) =>
          action.payload.some((account) => account.id === id),
        );

        if (state.selectedAccounts.length === 0 && action.payload.length > 0) {
          const preferred =
      ...
      .addCase(generatePostImage.fulfilled, (state, action) => {
        ...
        const { media, content, replaceUrl } = action.payload;

        if (replaceUrl) {
          state.images = state.images.filter((u) => u !== replaceUrl);
        }

        state.mediaLibrary = dedupeMediaLibrary([media, ...state.mediaLibrary]);
        if (!state.images.includes(media.url)) {
          state.images.push(media.url);
        }

        state.autoGeneratedImageUrl = media.url;
        state.lastAutoImageContent = content;
        state.generateImageError = null;
      })
      .addCase(generatePostImage.rejected, (state, action) => {
        state.generatingImage = false;
        state.generateImageError =
          (action.payload as string) ?? "Image generate nahi ho saki";
      })
